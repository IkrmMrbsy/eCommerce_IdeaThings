	...
	<div class="bg-lime-50">
			<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
					<div class="mx-auto max-w-2xl py-16 sm:py-24 lg:max-w-none lg:py-5">
							<h1 class="text-2xl font-bold text-gray-900">Koleksi Pakaian Pria</h1>
							<div class="mt-6 grid grid-cols-1 gap-y-12 sm:grid-cols-2 lg:grid-cols-3 lg:gap-x-6 lg:gap-y-12">
								...
								<div class="group relative transition-transform transform hover:scale-105">
										<div class="relative h-80 w-full overflow-hidden rounded-lg bg-white shadow-lg transition-shadow duration-300 ease-in-out group-hover:shadow-xl">
												<img src="{{ $product->img }}" alt="{{ $product->produk }}" class="h-full w-full object-cover object-center transition-transform duration-300 ease-in-out group-hover:scale-110">
										</div>
										...
										<div class="mt-4">
												<a href="{{ $product->link }}" target="_blank" class="block bg-lime-800 text-white text-center px-4 py-2 rounded-md hover:bg-lime-700 transition duration-200">Beli Sekarang</a>
										</div>
								</div>
								@endforeach
